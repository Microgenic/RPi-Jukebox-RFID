...
#
# Note: if this s your first time installing a phoniebox
...
"
echo "Hit ENTER to start the installation."
read INPUT
...
# Custom
git clone https://github.com/Microgenic/Jukebox.git
...
# Not pyserial not needed since we are using spi?
...
# copy custom GPIO script
echo "Copy custom GPIO script"
sudo cp /home/pi/RPi-Jukebox-RFID/misc/sampleconfigs/gpio-buttons.py.custom /home/pi/RPi-Jukebox-RFID/scripts/gpio-buttons.py
#sudo chmod +x /home/pi/RPi-Jukebox-RFID/scripts/gpio-buttons.py

# copy custom RFID script, since we use RC522 over SPI
echo "Copy custom RFID (RC522 over SPI) script"
sudo cp /home/pi/RPi-Jukebox-RFID/scripts/Reader.py.experimental /home/pi/RPi-Jukebox-RFID/scripts/Reader.py
...
sudo cp /home/pi/RPi-Jukebox-RFID/misc/sampleconfigs/phoniebox-i2c-lcd-display.service.stretch-default.sample /etc/systemd/system/phoniebox-i2c-lcd-display.service
...
sudo chown root:root /etc/systemd/system/phoniebox-i2c-lcd-display.service
...
sudo chmod 644 /etc/systemd/system/phoniebox-i2c-lcd-display.service
...
sudo systemctl enable phoniebox-i2c-lcd-display
